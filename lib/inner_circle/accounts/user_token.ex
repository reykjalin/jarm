defmodule InnerCircle.Accounts.UserToken do
  use Ecto.Schema
  import Ecto.Query
  ...
  def build_session_token(user) do
    ...
    {token, %InnerCircle.Accounts.UserToken{token: token, context: "session", user_id: user.id}}
  end
  ...
  defp build_hashed_token(user, context, sent_to) do
    ...
    {Base.url_encode64(token, padding: false),
     %InnerCircle.Accounts.UserToken{
       token: hashed_token,
       context: context,
       sent_to: sent_to,
       user_id: user.id
  ...
  def token_and_context_query(token, context) do
    from InnerCircle.Accounts.UserToken, where: [token: ^token, context: ^context]
  end
